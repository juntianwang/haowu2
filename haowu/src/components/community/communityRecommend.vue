	...
	<div class="recommend">
		...
		<div class="recommend-content">
			<ul v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
				<li v-for="(item,index) in initial" class="recommend-content-li">
...
<script>
	...
	export default {
		...
		methods:{
			...
			loadMore() {
				if(!this.elect.length){
					this.loading = false;
					this.nomore = true;
					return
				}
			  this.loading = true;
			  if(this.elect.length>=4){
			  	//每次加载4个
			  	for(let i=0;i<4;i++){
			  		this.initial.push(this.elect.splice(0,1)[0])
			  	}
			  }else if(this.elect.length<4){
		...
		mounted: function() {
			...
			axios.get('/community/recommend', {
				...
				that.initial = that.elect.splice(0,4);
