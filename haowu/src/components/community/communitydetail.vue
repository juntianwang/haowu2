	...
	export default {
		...
		methods: {
			...
			dogood() {
				...
				if (!this.gooded) {
					this.$store.state.onBol[this.$store.state.detailcommunity.index].likeBol = false;
				}
			},
			docollect() {
				...
				if(this.collected) {
					...
					this.$store.state.onBol[this.$store.state.detailcommunity.index].collectBol = false;
				}
